...
cd ~/catkin_ws/src
if [ -d /simulation/pennair2 ]; then
    echo "Using local copy of pennair2" >> /tmp/debug
    cp /simulation/pennair2 . -r
else 
    git clone https://github.com/pennaerial/pennair2.git
fi
cp /simulation/dragonfly . -r
cd ~/catkin_ws
...
do
    ...
    cp /tmp/debug /simulation/outputs/debug.txt
